...
do_legalization = False  # Module-level initialization
...
def partition_for_mrvl(
    ...
    **kwargs,
    ...
    """Partition the graph greedily into Marvell graph region(s) and a LLVM region. The LLVM
    ...
    # setup & register convert layerout options
    convert_layout_dict = {
        "qnn.avg_pool2d": ["NHWC"],
        "nn.conv2d": ["NHWC", "OHWI"],
        "nn.upsampling": ["NHWC"],
    ...
    seq_common_stage_1 = tvm.transform.Sequential(
        passes=[
    ...
    with tvm.transform.PassContext(opt_level=opt_level, disabled_pass=disabled_pass_list):
        tmp_mod1 = seq_common_stage_1(mod)
        ...
        tmp_mod1 = repartition_mrvl_subgraphs(tmp_mod1)  # , tvmc_debug_opts.force_llvm_id_list)
...
class MrvlPartitionGraphValidate(ExprVisitor):
    ...
    def validate_partitioned_graph(self, pre_quantized_model):
        ...
        if self.num_mrvl_partitions != 1 or self.num_llvm_partitions > 0:
            ...
            # If model is not pre-quantized, then raise a warning
            print(
...
def repartition_mrvl_subgraphs(mod):
    ...
    Un-partition those partitions which:
     ...
     - specified as force_llvm regions in debug options
    """
...
def add_attributes(mod, annotate_target_str, add_instrumentation, **kwargs):
    ...
    if "ocm_size" in kwargs:
        base_opts_str = f"{base_opts_str} -ocm_size={kwargs.get('ocm_size')}"
    elif "ocm_size" not in base_opts_str:
        base_opts_str = f"{base_opts_str} -ocm_size=0x100000"
...
def mrvl_pattern_table():
    ...
    def expand_dims_pattern():
        """Create an expand_dims pattern.
        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the expand_dims pattern.
        """
        pattern = is_op("expand_dims")(wildcard())
        return pattern

    def qnn_tanh_pattern():
        """Create a qnn.tanh pattern.
        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the qnn.tanh pattern.
        """
    ...
    def qnn_sigmoid_pattern():
        """Create a qnn.sigmoid pattern.
        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the qnn.sigmoid pattern.
        """
    ...
    def qnn_globalavgpool2d_pattern():
        """Create a globalavgpool2d pattern for qdq flow.
    ...
    def requantize_pattern():
        """Create a requantize pattern.
           review tvm/tests/python/relay/test_dataflow_pattern.py for examples

        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the requantize pattern.
        """
        pattern = is_op("qnn.requantize")(
            wildcard(), is_constant(), is_constant(), is_constant(), is_constant()
        )
    ...
    def qnn_mul_pattern():
        """Create a qnn.mul pattern.
        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the mul pattern.
        """
    ...
    def subtract_pattern():
        """Create a subtract pattern.
           review tvm/tests/python/relay/test_dataflow_pattern.py for examples
        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the subtract pattern.
        """
    ...
    def batch_matmul_pattern():
        """Create a batch_matmul pattern.
           review tvm/tests/python/relay/test_dataflow_pattern.py for examples

        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the batch_matmul pattern.
    ...
    def conv2d_transpose_pattern():
        ...
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the convolution-2d-transpose pattern.
        """
    ...
    def rsqrt_pattern():
        """Create a rsqrt pattern.
           review tvm/tests/python/relay/test_dataflow_pattern.py for examples

        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the rsqrt pattern.
        """
        pattern = is_op("rsqrt")(wildcard())
        return pattern

    def strided_slice_pattern():
        """Create a strided_slice pattern.
           review tvm/tests/python/relay/test_dataflow_pattern.py for examples

        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the strided_slice pattern.
        """
    ...
    def reduce_pattern():
        """Create a reduce pattern.
           review tvm/tests/python/relay/test_dataflow_pattern.py for examples

        Returns
        -------
        pattern : dataflow_pattern.AltPattern
            Denotes the reduce pattern.
        """
